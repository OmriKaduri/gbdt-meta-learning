...
def evaluate_metrics(model, X_test, y_test, is_multiclass):
    ...
    if is_multiclass:
        ...
        relevant_preds = test_preds_proba[:,np.unique(y_test)]
...
class HyperoptObjective(object):
    ...
    def __call__(self, hyper_params):
        ...
        # kf = kfold_good(self.X_train, self.y_train, 2)
        kf = StratifiedKFold(n_splits=2, shuffle=True)

        model = self.model(models, folds=kf.split(self.X_train, self.y_train), ** hyper_params, **self.constant_params)
...
def inner_cv_hyperopt(X, y, n_splits=3):
    ...
    parameter_space = {
        ...
        'RandomForestClassifier_est_1': hyperopt.hp.choice('RandomForestClassifier_est_1',
                                                           list(range(100, 500, 50))),
        'RandomForestClassifier_est_2': hyperopt.hp.choice('RandomForestClassifier_est_2',
                                                           list(range(100, 500, 50))),
        'ExtraTreesClassifier_est': hyperopt.hp.choice('ExtraTreesClassifier_est', list(range(100, 500, 50))),
        'GradientBoostingClassifier_est': hyperopt.hp.choice('GradientBoostingClassifier_est',
                                                             list(range(100, 500, 50))),
    }
    best_auc = 0
...
for cls_dataset in glob.glob('classification_datasets/*.csv')[6:]:
    ...
    record = {'Dataset': dataset_name, 'Algorithm': 'Pystacknet'}
